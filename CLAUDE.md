...
- **SEO meta is centralized**: `base.html` `<head>` includes `templates/includes/seo_meta.html`, driven by the `learning.context_processors.seo` defaults. To customize a page's description / OG image / canonical, set `meta_description` / `og_title` / `og_image` / `og_type` in the **view context** (don't hand-write meta tags in templates). For structured data, put a dict (or list of dicts) in the view context as `jsonld` — `base.html` renders it once in `<head>`; don't call `{% jsonld %}` in page templates. A `meta_robots` context key renders a `<meta name="robots">` (used by the search page for `noindex, follow`). Sitemap (`/sitemap.xml`, incl. module/lesson pages), `robots.txt` (with explicit AI-crawler allow groups), `llms.txt` (GEO: DB-generated Markdown site map), JSON-LD, and analytics are detailed in `docs/architecture.md` → **SEO / Discoverability**. New models that get their own page should add `get_absolute_url()` and, if public, a sitemap entry.
