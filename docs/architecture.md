...
- **Meta tags**: `templates/includes/seo_meta.html` (included from `base.html` `<head>`) renders the description, `<link rel="canonical">`, Open Graph, and Twitter `summary_large_image` cards. `og:image` defaults to `og-default.png` (1200x630 branded card — generate it with `.claude/skills/course-thumbnail/generate.py --og`, never hand-edit); course/lesson pages use `Course.get_thumbnail_url()` made absolute. `og:image:alt` / `twitter:image:alt` mirror `og_title`, so they stay accurate per page; `og:image:width/height` are deliberately **not** emitted — the image size varies per page (1200x630 default vs 1280x720 thumbnails vs YouTube thumbs), and wrong dimensions are worse than absent.
- **JSON-LD** via the `{% jsonld data %}` tag (`learning_extras.py`, escapes `<` so user strings can't break out of the `<script>`). Built in views under the `jsonld` context key (a dict **or list** of dicts) and rendered **once in `base.html` `<head>`** (`{% if jsonld %}{% jsonld jsonld %}{% endif %}` — templates no longer render it themselves):
  - Home → `WebSite` (with a `SearchAction` sitelinks search box pointing at `/malaka/qidiruv/?q=`) + `Organization` (`logo`, `sameAs` → the Telegram bot).
  - Course detail → `Course` (provider, `inLanguage=uz`, image, instructor, free `Offer`, `aggregateRating` when `rating_count`, `datePublished`, and `hasCourseInstance` with `courseMode=Online` + ISO-8601 `courseWorkload` — the piece Google requires for Course rich results) + `BreadcrumbList`.
  - Lesson detail → `VideoObject` for video lessons (thumbnail, embedUrl, ISO-8601 `duration`, `isPartOf` Course) or `Article` for article/konspekt lessons (author = `instructor_name`, publisher, `isPartOf`); quiz lessons emit none. Plus `BreadcrumbList`.
  - Module / category detail → `BreadcrumbList`. Learning-path detail → `ItemList` of its courses + `BreadcrumbList`. Instructor → `Person`.
  - Helpers in `learning/views.py`: `_breadcrumb_jsonld(*(name, path))`, `_iso_duration(seconds)`.
- **Sitemap** at `/sitemap.xml` (`learning/sitemaps.py`, `SITEMAPS`): static views, published courses, **modules and lessons of published courses** (the konspekt/lesson pages are public and are the site's richest indexable content), categories, learning paths, instructors. `lastmod` uses the course's `published_at` (deliberate: content is often edited via direct SQL, so an ORM `auto_now` field would lie). Domain comes from the request host (no `django.contrib.sites`); `protocol='https'`. Models expose `get_absolute_url()` for this.
- **robots.txt** at `/robots.txt` (`robots_txt` view in `config/urls.py`): disallows only `/admin/`; advertises the sitemap; and emits an explicit allow group per AI crawler (`AI_CRAWLERS` list: GPTBot, ClaudeBot, PerplexityBot, …) as a durable GEO signal. `/users/` and `/api/` are deliberately **crawlable** — see noindex below. (Cloudflare prepends its own managed/content-signal block ahead of ours on the live response — verify Cloudflare's **AI Crawl Control** isn't edge-blocking these bots.)
- **llms.txt** at `/llms.txt` (`llms_txt` view in `learning/views.py`, `@cache_page(3600)`): a Markdown map for AI assistants — site summary + links to every learning path, category and published course with one-line descriptions. Regenerates from the DB; nothing to maintain when content changes.
- **Internal search results** (`/malaka/qidiruv/`) set `meta_robots='noindex, follow'` in context; `seo_meta.html` renders `<meta name="robots">` only when that key is present. Any view can use it.
- **noindex on account/API paths** — `learning.middleware.NoindexMiddleware` stamps `X-Robots-Tag: noindex, nofollow` on every response under `/users/`, `/api/`, `/admin/`. `robots.txt` Disallow only blocks *crawling*, not *indexing*, so a Disallowed URL linked site-wide (e.g. the nav's `/users/login/`) still gets indexed bare from links — Search Console's "Indexed, though blocked by robots.txt". The fix is the inverse: leave `/users/` and `/api/` crawlable so Google reads the `noindex` and drops them; `/admin/` stays Disallowed (it 302-redirects anyway) but is stamped for defence in depth.
